...
namespace FilmRentalStore.Tests
{
    public class StoreTesting
    {
        ...
        public async Task GetStoreByCity_ValidCity_ReturnsOk()
        {
            ...
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(result);
            var returnedStores = Assert.IsType<List<StoreDTO>>(okResult.Value);
            Assert.Equal(expectedStores.Count, returnedStores.Count);
        }
        ...
        public async Task GetStoreByCountry_ValidCountry_ReturnsOk()
        {
            ...
            _storeRepositoryMock.Setup(repo => repo.GetStoreByCountry(country)).ReturnsAsync(expectedStores);
        ...
        {
            // Arrange
            var country = "";  // Invalid country name (empty string)
